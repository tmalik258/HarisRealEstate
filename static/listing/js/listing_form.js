$(document).ready(function () {
	...
	$(".nav-link").on("shown.bs.tab", function (e) {
		...
		var category = $(e.target.getAttribute("data-bs-target")).attr(
			"aria-labelledby"
		);
		switch (category) {
			case "home-tab":
				...
				$(
					"#furnished_state #furnished_field input#furnished_option1"
				).prop("checked", true);
				$(
					"#furnished_state #state_field input#construction-state-option1"
				).prop("checked", true);

				// Bedrooms and Bathrooms
				$("#beds_baths").css("display", "flex");
				$("#beds_baths #bedroom_field input#bedroom_option0").prop(
					"checked",
					true
				);
				...
				$('#floor_col #floor_field input[name="floor"]').each(function (
					_index,
					element
				) {
					// element == this
	...
	$('#floor_field input[name="floor"]').on("change", () => {
		...
		} else $("#id_custom_floor").css("display", "none");
	});
	...
	$("#id_price").on("input", function (e) {
		...
		if (val >= 1000000000) {
			$("#id_price_in_words").text(
				(val / 1000000000).toFixed(2) + " arab"
			);
		} else if (val >= 10000000) {
			$("#id_price_in_words").text(
				(val / 10000000).toFixed(2) + " crore"
			);
		} else if (val >= 100000) {
	...
	$("form").submit(function (e) {
		...
		// Purpose Field
		$("#id_purpose").val(
			$('#purpose_field input[name="purpose"]:checked').val()
		);

		// Furnished Field
		$("#id_furnished").val(
			$('#furnished_field input[name="furnished"]:checked').val()
		);

		// Construction State Field
		$("#id_state").val(
			$('#state_field input[name="construction-state"]:checked').val()
		);
...
function Category() {
	...
	switch (label_pane) {
		...
		case "plot-tab":
		case "commercial-tab":
			let input = $(tab_pane).find(
				`input[name="${label_pane}-pane"]:checked`
			);
			input_val = $(input).val();
			break;
		case "room-tab":
